...
const sourceWebparts  = "output/output-webpart-documents.txt"
const sourceMain      = "output/output-main-documents.txt"
const sourceDocumentAudit = "output/output-documentaudit-documents.txt"
const sourceImageAudit    = "output/output-imageaudit-documents.txt"
const sourcePagesAudit = "output/output-pageaudit-pages.txt"      // Get all pages so we can find parent page status

var validLinksFile   = "output/output-valid-links-documents.txt"
var brokenLinksFile  = "output/output-broken-links-documents.txt"
var validDocsFile    = "output/output-valid-documents.txt"
var unusedDocsFile   = "output/output-unused-documents.txt"

const outputFile    = "output/GatewayDocumentsAnalysis.xlsx"  

var ts = new Date();
console.log(ts.toLocaleTimeString(), " :: Start  - Compare Documents")
...
var ts = new Date();
console.log(ts.toLocaleTimeString(), " :: Finish - Compare Documents")
...
function loadData() {
  ...
  allLinks.sort();                                                                         // Sort
  console.log("    Total links to documents:               " + allLinks.length + "\n");
  ...
  console.log("    Documents Stored in Document folders:  " + filesInDocs.length);
  ...
  console.log("    Documents Stored in Image Libraries:     " + filesInImages.length);
  ...
  allFiles.sort();                                                                         // Sort
  console.log("    Total documents stored:                " + allFiles.length + "\n");
...
async function analyseData() {
  ...
  brokenLinksSheet.columns = [
    { header: 'Broken Link',                   key: 'fileurl',                    width: 70 },
    { header: 'Parent Page URL',               key: 'pageurl',                    width: 70 },
    { header: 'Location in Parent Page',       key: 'location',                   width: 20 },
    { header: 'Parent Page Moderation Status', key: 'parentstatus',               width: 20 },   
    { header: 'Parent Page Published Version', key: 'parentpagepublishedversion', width: 10 },
  ...
  var validDocsSheet = workbook.addWorksheet('Linked Documents');
  validDocsSheet.columns = [
    { header: 'File URL',          key: 'fileurl',          width: 100 },
    { header: 'File Type',         key: 'filetype',         width: 10 },
    { header: 'Modified',          key: 'modified',         width: 15 },
    { header: 'ModifiedBy',        key: 'modifiedBy',       width: 15 },
    { header: 'Moderation Status', key: 'moderationStatus', width: 20 },
    { header: 'Publishing Level',  key: 'publishingLevel',  width: 15 },
    { header: 'Checkout User',     key: 'checkoutTo',       width: 15 },
  ...
  var unusedDocsSheet = workbook.addWorksheet('Unlinked Documents');
  ...
  // Summary Text
  summarySheet.addRow(["Comparing all links in Content and Web Parts to Gateway Documents against all the Documents stored in Gateway"]).commit();
  ...
  summarySheet.addRow(["All links going to a document:",validLinks.length, pubPages, unpubPages]).commit();
  console.log("    All links going to a document:          " + validLinks.length + "  " + pubPages + "  " + unpubPages);
  ...
  summarySheet.addRow(["All broken links to documents:",brokenLinks.length, pubPages]).commit();
  console.log("    All broken links to documents:           " + brokenLinks.length + "     " + pubPages);
  brokenLinks.forEach(OutputBrokenLinks);

  // Get all files with 1 or more link to them
  var validDocs = compareArrays(allFiles, allLinks, "linked documents");
  var pubPages = countPublishedPages(validDocs, 9);
  summarySheet.addRow(["All documents with links to them:",validDocs.length, pubPages]).commit();
  console.log("    All documents with links to them:       " + validDocs.length + "  " + pubPages);
  validDocs.forEach(OutputValidDocs)

  // Get all files with no links to them
  var unusedDocs = compareArrays(allFiles, allLinks, "unlinked documents");
  var pubPages = countPublishedPages(unusedDocs, 9);
  summarySheet.addRow(["All documents with no links to them:",unusedDocs.length, pubPages]).commit();
  console.log("    All documents with no links to them:    " + unusedDocs.length + "   " + pubPages);
  ...
  function OutputBrokenLinks(record) {
    ...
    fs.appendFileSync(brokenLinksFile, '<li><a href=' + record[0] + '>' + record[0] + '</a></li>');  
  ...
  function compareArrays(arrayA, arrayB, option) {
    ...
    switch (option) {
      ...
      case "linked documents":  // All documents with at least one link to them
        for (i = 0; i < arrayA.length; i++) {
          found = false;
          linkCount = 0
          for (j = 0; j < arrayB.length; j++) {
      ...
      case "unlinked documents":   // All documemnts that have no links to them
        for (i = 0; i < arrayA.length; i++) {
          found = false;
          for (j = 0; j < arrayB.length; j++) {
            if (arrayA[i][0].toLowerCase() === arrayB[j][0].toLowerCase()) {
